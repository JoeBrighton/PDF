...
def parse_shiftkey_invoice(pdf_bytes):
    ...
    ITEM_RE = re.compile(
        MONTHS + r'\s+\d+\w*,\s+\d{4}\s+'
        r'(' + MONTHS + r'\s+\d+\w*,\s+\d{4})\s+'
        r'(CNA|RN|LPN|LVN)\s+'
        r'([A-Za-z\-\']+,\s+[A-Za-z\-\' ]+?)\s+'
...
def build_flags_excel(meta, recon_rows):
    ...
    def row_fill(flag):
        ...
        if 'TIME DIFF' in flag:    return S['YLW_F']
    ...
    ws1["A1"] = f"Invoice {meta['invoice']} — {meta['facility']} — {meta.get('vendor','')}"
    ...
    stat_labels = ["Match", "No Punch", "Overbilled", "Underbilled", "Other", "Late Cancel",
                   "Inv Hours", "Punch Hours", "Punch Disc.", "Missing Hrs"]
    ...
    day_hdrs = ["Date","Shifts","Inv Hrs","Punch Hrs","Hrs Diff",
    ...
    for day in sorted(day_map.keys(), key=lambda d: (d or '')):
        ...
        n_other  = max(len(day_rows) - n_match - n_np - n_over - n_under - n_td - n_lc, 0)
        if n_np or n_over:
            rf3 = S['RED_F']
        ...
        for ci, val in enumerate(vals, 1):
            ...
            c.fill = rf3; c.alignment = S['CTR']; c.font = S['NF']
            if ci == 1: c.font = S['BF']
            if ci == 5 and val:
                c.number_format = "+0.00;-0.00;0.00"
                c.font = S['RF'] if val > 0.05 else (S['BLF'] if val < -0.05 else S['NF'])
        ws3.row_dimensions[ri3].height = 16; ri3 += 1
    ...
    for i, w in enumerate([10,7,9,9,9,8,9,10,11,10,7,11], 1):
...
def parse_intact_excel(xlsx_bytes):
    ...
    for row in ws.iter_rows(min_row=hdr_row+1, values_only=True):
        ...
        if ck is not None:
            try: ck = str(int(float(ck)))
            except (ValueError, TypeError): ck = str(ck).strip() or None
        ...
        rows.append({'check_no': ck, 'amount': amt, 'vendor': vendor,
                     'gl_date': gl_date, 'txn_type': txn_type, 'txn_sub': txn_sub})
    return rows
...
def parse_cibc_statement(pdf_bytes):
    ...
    while i < len(lines):
        ...
        for j in range(i+1, min(i+3, len(lines))):
            c = lines[j].strip()
            if DATE_AMT_RE.match(c) or not c: break
            cont_lines.append(c)
        ...
        if ck_m:
            ...
            transactions[ck_no] = {'date': date, 'check_no': ck_no,
                'vendor_raw': vendor_raw, 'amount': amount, 'type': 'ACH Check', 'ref_id': ref_id}
            ...
            transactions[key] = {'date': date, 'check_no': None,
                'vendor_raw': vend_raw, 'amount': amount, 'type': 'Wire', 'ref_id': ref_id}
        elif 'Cash Mgmt Trsfr' in desc1:
            key = f"XFER_{date}_{len(transactions)}"
            transactions[key] = {'date': date, 'check_no': None,
                'vendor_raw': full_desc, 'amount': amount, 'type': 'Transfer', 'ref_id': ref_id}
        else:
            key = f"ACH_{date}_{len(transactions)}"
            transactions[key] = {'date': date, 'check_no': None,
                'vendor_raw': full_desc, 'amount': amount, 'type': 'ACH/Debit', 'ref_id': ref_id}
        i += 1
    CHK_RE = re.compile(r'Check\s+#(\d+),\s+Posted(\d{2}/\d{2}/\d{2}),\s+Amount([\d,]+\.\d{2})')
    for m in CHK_RE.finditer(text):
        ck_no = str(int(m.group(1)))
        transactions[ck_no] = {'date': m.group(2)[:5], 'check_no': ck_no,
...
def parse_regions_statement(pdf_bytes):
    ...
    for line in text.split('\n'):
        ...
        if ck_m:
            ...
            transactions[ck_no] = {'date': date, 'check_no': ck_no,
                'vendor_raw': vendor_raw, 'amount': amount, 'type': 'ACH Check', 'ref_id': None}
        elif 'Wire Transfer' in desc:
            key = f"WIRE_{wire_idx}"; wire_idx += 1
            transactions[key] = {'date': date, 'check_no': None,
                'vendor_raw': desc.replace('Wire Transfer', '').strip(),
                'amount': amount, 'type': 'Wire', 'ref_id': None}
        else:
            ref_m = re.search(r'(?<!\d)(\d{8,})(?!\d)', desc)
            ref_id = ref_m.group(1) if ref_m else None
            key = f"ACH_{date}_{len(transactions)}"
            transactions[key] = {'date': date, 'check_no': None,
                'vendor_raw': desc, 'amount': amount, 'type': 'ACH/Debit', 'ref_id': ref_id}
    in_checks = False
    ...
    for line in text.split('\n'):
        ...
        if 'Total Checks' in s: in_checks = False
        if in_checks:
            for m in CHK_SEC_RE.finditer(s):
                ck_no = m.group(2)
                transactions[ck_no] = {'date': m.group(1), 'check_no': ck_no,
                    'vendor_raw': '', 'amount': float(m.group(3).replace(',', '')),
                    'type': 'Check', 'ref_id': None}
    return transactions
...
def match_intact_to_bank(intact_rows, bank_txns):
    ...
    for row in intact_rows:
        ...
        if best_b and best_score >= 0.4:
            ...
            used_bank.add(id(best_b)); matched_intact.add(id(row))
    # Pass 3: amount only
    for row in intact_rows:
        if id(row) in matched_intact: continue
        amt = row.get('amount')
        ...
        for b in bank_txns.values():
            ...
            if abs(b['amount'] - amt) < 0.02:
                matched.append({'intact_row': row, 'bank': b, 'match_type': _criteria(row, b)})
                used_bank.add(id(b)); matched_intact.add(id(row)); break
    # Pass 4: split bank transactions
    ...
    for row in intact_rows:
        ...
        for ref_id, group in ref_groups.items():
            if abs(sum(b['amount'] for b in group) - amt) < 0.02:
                primary = max(group, key=lambda b: b['amount'])
                matched.append({'intact_row': row, 'bank': primary,
                    'match_type': f'split ({len(group)} bank txns, ref {ref_id})',
                    'split_group': group})
                for b in group: used_bank.add(id(b))
                matched_intact.add(id(row)); break
    return matched
...
with tab2:
    ...
    st.caption("Upload the GL register Excel and your bank statement PDF.")

    ap1, ap2 = st.columns(2)
    with ap1:
        intact_file = st.file_uploader("GL Register (Excel)", type=["xlsx", "xls"], key="intact")
    with ap2:
    ...
    if intact_file and bank_file:
        if st.button("Run AP Reconciliation", type="primary", use_container_width=True):
            with st.spinner("Parsing GL export..."):
            ...
            mc1.metric("GL items",          len(intact_rows))
            ...
            if pending:
                ...
                st.dataframe(pd.DataFrame([
                    {'Vendor': v, 'Items': vendor_counts[v], 'Outstanding': f"${amt:,.2f}"}
                    for v, amt in vendor_summary
                ]), use_container_width=True, hide_index=True)
            else:
                st.success("All items cleared!")
            matched_bank_ids = {id(m['bank']) for m in matches}
            unmatched_bank = [v for v in bank_txns.values() if id(v) not in matched_bank_ids]
            if unmatched_bank:
                st.divider()
                st.subheader(f"{len(unmatched_bank)} Bank Transaction(s) Not in GL")
                st.caption("These cleared the bank but have no matching GL entry.")
                st.dataframe(pd.DataFrame([{
                    'Bank Date': t['date'], 'Check #': t['check_no'] or '--',
            ...
            wb2 = WB2(); ws_ap = wb2.active; ws_ap.title = "AP Reconciliation"
            GREEN = PatternFill("solid", start_color="C6EFCE")
            RED_F2 = PatternFill("solid", start_color="FFC7CE")
            hdrs = list(all_rows[0].keys())
            for ci, h in enumerate(hdrs, 1):
                c = ws_ap.cell(row=1, column=ci, value=h)
                c.font = Font(bold=True, name="Arial", size=9, color="FFFFFF")
                c.fill = PatternFill("solid", start_color="1F4E79")
            for ri_ap, row in enumerate(all_rows, 2):
                fill = GREEN if row['Status'] == 'CLEARED' else RED_F2
                for ci, key in enumerate(hdrs, 1):
                    c = ws_ap.cell(row=ri_ap, column=ci, value=row[key])
                    c.fill = fill; c.font = Font(name="Arial", size=9)
            for ci in [1,2,3]: ws_ap.column_dimensions[get_column_letter(ci)].width = 12
            ws_ap.column_dimensions["D"].width = 32
            for ci in [5,6,7]: ws_ap.column_dimensions[get_column_letter(ci)].width = 14
            ws_ni = wb2.create_sheet("Not in GL")
            ni_hdrs = ['', 'Bank Date', 'Type', 'Check #', 'Vendor', 'Amount', 'Notes']
            for ci, h in enumerate(ni_hdrs, 1):
                c = ws_ni.cell(row=1, column=ci, value=h)
                c.font = Font(bold=True, name="Arial", size=9, color="FFFFFF")
                c.fill = PatternFill("solid", start_color="1F4E79")
            YELLOW = PatternFill("solid", start_color="FFEB9C")
            for ri_ni, t in enumerate(sorted(unmatched_bank, key=lambda x: x['amount'], reverse=True), 2):
                ws_ni.cell(row=ri_ni, column=1, value='☐').font = Font(name="Arial", size=11)
                ws_ni.cell(row=ri_ni, column=2, value=t['date']).font = Font(name="Arial", size=9)
                ws_ni.cell(row=ri_ni, column=3, value=t['type']).font = Font(name="Arial", size=9)
                ws_ni.cell(row=ri_ni, column=4, value=t['check_no'] or '--').font = Font(name="Arial", size=9)
                ws_ni.cell(row=ri_ni, column=5, value=t['vendor_raw'] or '--').font = Font(name="Arial", size=9)
                ac = ws_ni.cell(row=ri_ni, column=6, value=t['amount'])
                ac.font = Font(name="Arial", size=9); ac.number_format = '"$"#,##0.00'
                nc = ws_ni.cell(row=ri_ni, column=7, value='')
                nc.fill = YELLOW; nc.font = Font(name="Arial", size=9)
            for col, w in zip(["A","B","C","D","E","F","G"], [4,10,12,10,40,14,30]):
                ws_ni.column_dimensions[col].width = w
            ws_ni.freeze_panes = "A2"
            wb2.save(out); out.seek(0)
            st.download_button("Download Reconciliation (Excel)", data=out.read(),
...
with tab1:
    ...
    if pdf_files and xlsx_file:
        if st.button("Run Reconciliation", type="primary", use_container_width=True):
            ...
            vendors   = ', '.join(sorted({m.get('vendor', '') for m in all_metas if m.get('vendor')}))
            ...
            if flag_rows:
                ...
                table_data = [{
                    ...
                    "Punch Hrs": f"{r['e_adj']:.2f}" if r['e_adj'] is not None else "--",
                    "Hrs Diff":  f"{r['hrs_diff']:+.2f}" if r['hrs_diff'] is not None else "--",
                    "Inv In":    fmt_dt(r['start']),
                    "Inv Out":   fmt_dt(r['end']),
                    "Punch In":  fmt_dt(r['e_in']),
                    "Punch Out": fmt_dt(r['e_out']),
                    "Notes":     r['notes'],
                } for r in flag_rows]
            ...
            inv_slug = all_metas[0].get("invoice", "invoice").replace("-", "_")
            fac_slug = re.sub(r"[^a-zA-Z0-9]", "_", all_metas[0].get("facility", "facility"))[:20].strip("_")
            d1, d2 = st.columns(2)
            with d1:
                hours_bytes = build_hours_excel(all_metas[0], all_shifts)
                st.download_button("Hours by Employee (Excel)", data=hours_bytes,
                    file_name=f"{fac_slug}_{inv_slug}_Hours.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True)
            with d2:
                flags_bytes = build_flags_excel(all_metas[0], recon_rows)
                st.download_button("Flags & Punch-for-Punch (Excel)", data=flags_bytes,
